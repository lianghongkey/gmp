...
速度：101.25 MHz 上 decode 一步实测 0.065 秒（15.45 token/s）；64 个 token 的 prefill 一轮
52,162,834 拍，0.52 秒。编译器导出的七份判据在板上全部通过，其中整网那一份跑满 65 轮
（1 轮 prefill 加 64 步 decode），logits 与 token 两项与 golden 逐位相同。

布局布线之后的片上资源占用，时序按 101.25 MHz 的 9.877 ns 周期收敛，布线后最差建立裕量 +0.064 ns：

| 资源 | 用量 | 器件总量 | 占比 |
| - | -: | -: | -: |
| LUT | 215,974 | 298,600 | 72.3% |
| 触发器 | 157,466 | 597,200 | 26.4% |
