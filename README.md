...
速度：decode 一步 12,184,243 拍，50 MHz 上是 0.24 秒（4.1 token/s）；64 个 token 的 prefill
一轮 54,863,944 拍，1.10 秒。编译器导出的七份判据在板上全部通过，其中整网那一份跑满 65 轮
（1 轮 prefill 加 64 步 decode），logits 与 token 两项与 golden 逐位相同。

布局布线之后的片上资源占用，时序按 19 ns 收敛（比板上 20 ns 的时钟周期紧 5%），布线后最差
建立裕量 +0.220 ns：

| 资源 | 用量 | 器件总量 | 占比 |
| - | -: | -: | -: |
| LUT | 256,676 | 298,600 | 86.0% |
| 触发器 | 187,770 | 597,200 | 31.4% |
| BRAM（按 RAMB36 折算） | 483.5 | 955 | 50.6% |
...
控制器，右上角那一小块是 1GbE 的链路层。SLICE 已经占到 99%，片上可编程逻辑用满了。
