...
要等多久、哪些地方与板上不一样、出错先看什么。板上那条路在 [`usage.md`](usage.md)。
...
| 内存 | 权重灌完仿真进程占约 1.1 GB，峰值 1.4 GB，机器留 2 GB 富余 |
| CPU | 仿真自己开 8 个线程，核越快跑得越快，机器上同时跑着别的重负载时会按比例变慢 |
| Python | 与板上同一套，只要 numpy。不用 pyusb、不用 udev 规则、不用 Vivado |
...
```bash
python host/soc_generate.py --sim --check-only --check-weights 0 "今天天气不错，我们去"
```

打出“仿真里认出这颗 SoC”就算通了，那一行同时报这台机器每秒打多少拍。

再跑生成，就是板上那条命令加 `--sim`：

```bash
python host/soc_generate.py --sim --max-new 1 "今天天气不错，我们去"
python host/soc_generate.py --sim --chat "请用一句话介绍一下你自己。"
```

要等多久由步数决定：喂 prompt 一个 token 一步，生成一个 token 一步（prompt 正好满 64 个时
前 64 个并成一轮 prefill）。一步 decode 是 2471 万拍，机器空着时每秒能打 10 万拍上下，
一步就是三到四分钟；机器上有别的负载跟它抢核时，一步会成倍地长。`--max-new` 限制生成几个，
头一趟先跑一两个看看。

算出来的东西与板上一样：`usage.md` 里板上那趟的 prompt“今天天气不错，我们去”，在仿真上
喂完六个 token 之后片上给出的下一个 token 同样是 102077“公园”，与板上、与 llama.cpp 的
贪心解码逐字相同。六步花了 1466 秒。
...
── 仿真 ──
    仿真起来了：prebuilt/sim/VSocCosimTop（pid 3881514），它打印的东西落在 out/sim_gmp_soc_axi.log
    仿真里认出这颗 SoC，空转 94 k 拍/秒（一步 decode 2470 万拍，这个速率下约 4 分钟）
── 灌整份权重（604 MiB，几秒钟）──
      … 158 MiB（644.44 MB/s，还要约 0 分钟）
      …（每 64 MiB 报一行）
      … 576 MiB（428.51 MB/s，还要约 0 分钟）
    权重 604.0 MiB 灌完，0.0 分钟
    权重抽查 16 处一致
    清零：64 段，共 14.0 MiB，0 s
    CPU 报到，程序版本 1
    逐个喂 2 个：1051.9 s
...
这两步走了 1052 秒，比开头那行按空转速率估的四分钟慢一倍多，慢的原因是与别的负载抢核。最后那个
token 是喂 prompt 最末一轮顺带算出来的，没有再多跑一步 decode，所以“decode 平均”那栏是 0。
...
| 往 DRAM 搬字节 | 经片上互联，604 MiB 要 18 分钟 | 走后门直接放进 DRAM 模型，不占仿真拍，604 MiB 半分钟内灌完（`weights.npz` 还在页缓存里时几秒） |
| 一步 decode | 0.49 秒 | 三到四分钟；CPU 报的拍数是 2471 万，按 50 MHz 折算正是板上那 0.49 秒 |
...
| 比上面报的时间慢很多 | 机器上有别的负载，仿真会把分到的核跑满。`--check-only` 那条命令报的每秒拍数就是这台机器当下的实际速度 |
